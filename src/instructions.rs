...
pub fn build_initial_instructions(config: &AppConfig) -> String {
    ...
    [
        ...
        "A ChatGPT conversation cannot switch between projects or scratch after selection; start a new chat for another choice. Clients without stable `openai/session` metadata fall back to an MCP-transport-session binding and must select again after reconnecting; transport scratch is deleted when that session ends.".to_string(),

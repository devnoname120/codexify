...
pub const AGENT_BRIEF: &str = concat!(
    ...
    "\n## Special requests\n\n",
    "- Call `self_update` only after the user explicitly asks to update Codexify. Pass `confirm=true`; when the server is service-supervised, warn that MCP will disconnect temporarily while the detached updater stops and restarts the service.\n",

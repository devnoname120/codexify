...
pub const AGENT_BRIEF: &str = concat!(
    ...
    "- After the final related file change, call show_diff once to present the project-scoped aggregate diff. By default it records the emitted snapshot as the next incremental baseline; this is private connector bookkeeping and does not modify the project. Use advance=false only when the user asks for a non-advancing inspection.\n",
    "\n## Running commands\n\n",
    ...
    "- Call remember when you learn something that cost you effort and would cost it again: a decision and its reason, a constraint that is not visible in the code, where something unexpected lives, an approach you tried that did not work. Keep each note to a sentence or two. remember creates a new key; use update_memory_note to replace one and forget_memory_note to remove one.\n",
    ...
    "\n## Special requests\n\n",
    "- Call `self_update` only after the user explicitly asks to update Codexify. Pass `confirm=true`; when the server is service-supervised, warn that MCP will disconnect temporarily while the detached updater stops and restarts the service. After a scheduled update, explicitly tell the user to open ChatGPT Settings, select the Codexify connector, scroll to the bottom of its tool list, and click Refresh after Codexify restarts so ChatGPT reloads the connector tools.\n",
    "- Direct self-update progress checks to `codexify service logs -f`.\n",
...
pub fn build_initial_instructions(config: &AppConfig) -> String {
    ...
    [
        ...
        String::new(),
        "This server is running in multi-project mode. Workspace identity cannot be resolved during MCP initialization because stable ChatGPT conversation metadata arrives on tool calls.".to_string(),
        format!("Project access root: {}", config.work_dir.display()),
        format!("Git clone directory: {}", config.project_clone_dir.display()),
        "For ChatGPT, a project or scratch workspace selected earlier in this same conversation is restored automatically even after an MCP reconnect or server restart. A new conversation has no binding.".to_string(),
        crate::tools::set_project_root::SetProjectRoot::RESUME_GUIDANCE.to_string(),
        "Call `get_agent_brief` first. If this conversation is not yet bound and the user supplied an exact filesystem path, an HTTPS/SSH Git repository URL ending in `.git`, or a supported GitHub repository, branch, pull-request, or commit URL, call `set_project_root` directly. A repository URL reuses a matching checkout or clones it into the configured clone directory; targeted GitHub URLs select the exact target without moving an unrelated source checkout. If the user explicitly wants no source project, call `set_project_root` with `withoutProject=true` to create a private scratch workspace. When project intent is ambiguous or the user only says hello, leave the setup project picker visible and let the user search/select. Do not choose scratch to unlock chat tools. You may use `list_projects` when a project name was provided; select automatically only when that intent is unambiguous. `chat_await` can wait for the picker without selecting a workspace. When selecting a project, an explicit user worktree preference overrides configuration: pass `createWorktree=false` if the user asks not to create a worktree, or `createWorktree=true` if they request one. Omit the argument only when no preference was expressed. After any selection, call `get_agent_brief` again. Repeating the same choice is idempotent.".to_string(),
        "Only an explicit user action in the setup card can reopen project selection with Switch to another project. After the choice changes, inspect workspace_changed and load get_agent_brief before doing any more project work. Do not reuse old paths or instructions; prior files/worktrees and processes are preserved. Clients without stable `openai/session` metadata fall back to an MCP-transport-session binding and must select again after reconnecting; the old transport binding is forgotten, but its scratch workspace and files remain on disk.".to_string(),

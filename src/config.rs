...
mod tests {
    ...
    fn widget_debug_is_configurable_and_legacy_settings_url_is_ignored() {
        ...
        std::fs::write(
            ...
            serde_json::to_vec(&json!({
                "workDir": root.path(),
                "debug": true,
                "chatgptConnectorSettingsUrl": "https://chatgpt.com/g/example/project#settings/Plugins/plugin_example",
                "codexMcp": { "enabled": false }
            }))
            .unwrap(),
        )
        ...
        std::fs::write(
            ...
            serde_json::to_vec(&json!({
                "workDir": root.path(),
                "chatgptConnectorSettingsUrl": "https://example.com/not-chatgpt",
                "codexMcp": { "enabled": false }
            }))
            .unwrap(),
        )
    ...
    #[test]
    fn ui_widgets_default_on_and_can_be_disabled() {
        let root = tempfile::tempdir().unwrap();
        assert!(default_config(root.path().to_path_buf()).ui_widgets);

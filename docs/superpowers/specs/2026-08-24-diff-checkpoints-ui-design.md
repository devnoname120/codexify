# Diff checkpoints and MCP Apps UI design

## Goal

Add an explicit diff workflow to Codexify without coupling checkpoint correctness to a particular MCP host UI. The server must be able to report changes since project opening or since the last emitted diff, advance the incremental baseline atomically, and optionally render the complete diff as an MCP App without placing the diff in model-visible context.
...
1. The logical project root is the diff scope. A project nested in a monorepo must never include sibling changes.
...
4. The last-diff checkpoint advances with compare-and-swap semantics when a diff snapshot is emitted, so concurrent diffs cannot silently overwrite one another.
...
7. The ordinary MCP content is concise and model-visible. Complete diff data is carried only in namespaced result `_meta`, which MCP Apps receive but the model does not.
8. `show_diff` is host-facing read-only even though it updates a connector-private incremental-delivery cursor. Cursor advancement completes before the result is returned and never depends on whether the user expands, collapses, or otherwise interacts with the widget.
...
Installations that already have the former review-named refs are migrated lazily.
When a current diff ref is absent, Codexify copies the corresponding
`refs/codexify/review/.../project-open` or `.../last-review` commit into the new
namespace. The old refs are retained so rolling back the executable does not lose
the existing incremental baseline.
...
Checkpoint initialisation is best-effort for unrelated tools: a non-Git project must still be readable and editable. `show_diff` reports the underlying Git error explicitly.

`show_diff` accepts:

- `since: "last_diff" | "project_open"`, default `last_diff`;
- `advance: boolean`, default `true`;
- `include_patch: boolean`, default `true`.

It creates one current snapshot, compares it with the requested baseline, and by default records that emitted snapshot as `last-diff` through `git update-ref <ref> <new> <expected-old>`. A compare-and-swap conflict preserves the returned diff but reports that the incremental cursor was not advanced. `advance=false` leaves the cursor unchanged. The cursor controls only which changes the connector emits on the next incremental call; it is not project, Git-history, user-owned, or external state, so the tool remains annotated read-only and idempotent.
...
A patch larger than `diff.maxPatchBytes` is omitted rather than cut mid-hunk. File metadata and aggregate statistics remain available. The default is 4 MiB, regression-tested with 10,000 changed code lines whose source lines are roughly 300 bytes long. Byte bounding remains necessary because line length and binary-patch expansion are unbounded. `0` disables patch bodies.
...
The `show_diff` tool descriptor links that resource through both the current nested `_meta.ui.resourceUri` shape and the compatibility `_meta["ui/resourceUri"]` key. Its nested UI visibility is model-only because the component never calls tools. The tool result stores the complete `DiffResult` under `_meta["io.github.devnoname120/codexify/diff"]`. Unsupported clients ignore the component metadata and continue to receive the concise text result. Existing cards using `_meta["io.github.devnoname120/codexify/review"]` or the `ui://codexify/review/v3`, v2, and unversioned resource URIs remain readable. The widget also maps the former `last_review` value and `reviewOpen` private state to their diff-named equivalents. Current tool results emit only the diff-named contract and never populate model-visible structured content.
...
Checkpoint initialization, mutating tool calls, and diffs for one owner/project pair are serialized in-process through completion of the tool call. Git refs provide cross-process atomicity for persistent checkpoint creation and cursor advancement. A resident process returned by `exec_command` can outlive its initiating call, so later filesystem mutations from that process are intentionally observed as point-in-time state rather than held behind an unbounded diff lock. Snapshot and diff subprocesses run with Codexify's secret-scrubbing policy and controlled temporary-index/identity variables.

Failure rules:

- not a Git worktree: `show_diff` returns a tool error;
- both missing baseline refs are initialized from the current scoped state; a missing last-diff ref is restored from project-open, while last-diff without project-open fails closed;
- oversized patch: success with explicit omission;
- concurrent baseline advancement: the diff is returned, the cursor is not advanced, and `show_diff` includes an explicit warning;
- non-Git projects: record diff as unavailable and continue normal tools;
...
- immutable project-open and advancing last-diff baselines;
...
- serialization between mutating tool calls and diffs;
